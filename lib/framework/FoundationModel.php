...
class FoundationModel {
	...
	public function validate_greater_than($thing_to_vaidate, $gt){
            $properties = get_object_vars($this);
            $value = $properties[$thing_to_vaidate];

            if (intval($value) < intval($gt) ){
                $this->_validate_errors[$thing_to_vaidate] = $thing_to_vaidate . " must be greater than $gt";
                return false;
            }
	...
	public function validate_less_than($thing_to_vaidate, $lt){
            $properties = get_object_vars($this);
            $value = $properties[$thing_to_vaidate];

            if (intval($value) > intval($lt) ){
                $this->_validate_errors[$thing_to_vaidate] = $thing_to_vaidate . " must be less than $lt";
                return false;
            }
	...
	public function validate_in_range($thing_to_vaidate,  $gt, $lt){
            $properties = get_object_vars($this);
            $value = $properties[$thing_to_vaidate];

            if($this->validate_greater_than($thing_to_vaidate, $gt) && $this->validate_less_than($thing_to_vaidate, $lt) ){
                return true;
            }
